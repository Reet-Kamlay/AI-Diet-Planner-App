...
import { Coffee02Icon, HugeiconsIcon, Moon02Icon, Sun03Icon } from '@hugeicons/core-free-icons';
...
export default function AddToMealActionSheet({recipeDetail,hideActionSheet}) {
    ...
    const CreateMealPlan=useMutation(api.AddToMealPlan.CreateMealPlan)
  ...
  return (
    <TouchableOpacity onPress={() => setSelectedDate(item)} style={{
      ...
      <FlatList data={mealOptions} 
      numColumns={4}
      renderItem={({item, index}) => {
  return (
    <TouchableOpacity onPress={() => setSelectedMeal(item?.title)} style={{
      flex: 1,
      display: 'flex',
      alignItems: 'center',
      padding: 7,
      ...
      <View style={{
        ...
        <TouchableOpacity onPress={()=>hideActionSheet()} style={{
            padding:15
        }}>
            <Text style={{
                textAlign:'center',
